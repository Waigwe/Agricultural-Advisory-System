...
class FirestoreService {
 ...
 // sesnors collection reference
  ...
  Future updateUserData(String username, String email, String phoneNumber, String location, String photoUrl) async {
   ...
   Future updateResponsesData(String name,  String avatarUrl, String forumId, String timestamp, String response, int type) async {
    return await responsesCollection.document(forumId)
    .collection(forumId).document(timestamp)
      .setData({
      'uid': uid,
      'name': name,
      'avatarUrl': avatarUrl,
 ...
 // get forum stream
  ...
  Stream<List<Responses>> get responses {
    return responsesCollection.document(uid).collection(uid).orderBy('timestamp', descending:true)
    .snapshots()
