...
class _ForumScreenPageState extends State<ForumScreenPage> {
  _ForumScreenPageState({Key key, @required this.currentforum});
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      .document(currentforum.timestamp)
      .collection(currentforum.timestamp)
      .orderBy('timestamp', descending: true)
      .snapshots(),
      builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
        if(snapshot.hasData){
          responseList = snapshot.data.documents;
          // responseList.forEach((doc) => print(doc));
          return  Flexible(
                                      ...
                                      Container(
                                        ...
                                        CircleAvatar(
                                          ...
                                          CachedNetworkImageProvider(
                                            responseList[index]['avatarUrl'].toString()
                                          ...
                                          Column(
                                           mainAxisAlignment: MainAxisAlignment.start,
                                            children: <Widget>[
                                          Text(
                                           responseList[index]['name'].toString(),
                                          ...
                                          Text(
                                            DateFormat('dd MMM kk:mm')
                                              .format(
                                                DateTime.fromMillisecondsSinceEpoch(int.parse(responseList[index]['timestamp'].toString())
                                                )
                                                ),                                            
                                            textAlign: TextAlign.start,
                                            maxLines: 1,
                                          ...
                                          children: <Widget>[
                                            Column(
                                              children: <Widget>[
                                            Text(
                                              responseList[index]['response'].toString(),
                                               maxLines: 4,
                                             textAlign: TextAlign.start,
                                          style: TextStyle(
